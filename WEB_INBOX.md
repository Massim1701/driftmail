...
**Umgesetzt (Terminal, 25.09., Commit `e0b0130`): iOS.** Bestehendes Icon (siehe voriger Eintrag) angepasst: helle Fluegel-Flaeche auf `#8ecbf5`, neue dunkelste Heck-Zipfel-Facette `#004578` ergaenzt (kurzer spitzer Zipfel hinten unten). Kein Stockfoto verwendet -- eigenes SVG-Artwork per Headless-Browser-Rendering nach genau dieser Textbeschreibung neu gezeichnet. `contracts/design-tokens.json` `systemFolders.facetIconStyle.gesendet` entsprechend aktualisiert (gemeinsame Referenz fuer Track F). **Offen: Web (Track F)** -- dasselbe Icon fuer die Web-Sidebar.
