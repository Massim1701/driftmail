...
[2026-09-08] [erledigt: b6b3eb2 (Contract-Felder in api-spec.yaml), Track B Erkennung: erledigt, siehe SYNC.md Track-B-Eintrag auf Branch track-b-security (`security-classification/src/draftPhishingCheck.ts`, inkl. `creditCardDetection.ts`/`credentialRequestLanguage.ts` fuer sensible Daten; `recipientReputation` bleibt bewusst immer "unknown", siehe neue offene Frage in SYNC.md -- DB-Zugriff auf fraud_alerts noetig, den dieses Modul nicht hat), Track A/UI Logik (Empfaenger-Reputations-Lookup + Sprechblasen-UI) folgt] [Erweiterung des Phishing-Check-Eintrags von eben, contracts/api-spec.yaml + Track A/UI] — Drei zusaetzliche Signale fuer denselben "Check vor dem Senden"-Moment (POST /messages/draft/phishing-check), NICHT als harter Block wie Phishing, sondern als nicht-blockierender Warnhinweis (Sprechblase/Tooltip nahe der betroffenen Textstelle):
