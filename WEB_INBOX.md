...
[2026-09-09] [teilweise erledigt: Punkt 1 = Commit cc56438, Punkt 2 = offen] [KORREKTUR der letzten Regel + neue Ergaenzung] [Track A/B + Track C/F] — Massimo: zwei zusammenhaengende Punkte, beide durchdacht statt isoliert umzusetzen:
...
[2026-09-22] [erledigt: Backend siehe SYNC.md] [NEUE GRUNDLAGE - HTML-Rendering des Mail-Bodies] [contracts + Track A/B] [hohe Prioritaet, Grundlage fuer mehrere vorher dokumentierte Luecken] — Massimo: nach dem zentralen Impressum als Naechstes das HTML-Rendering des Mail-Bodies, danach Design-Richtung/Malware-Scan/UI fuer die neun Features (letztere drei waren zu diesem Zeitpunkt bereits erledigt, siehe fruehere Eintraege). Backend-Umsetzung: `messages.body_html` (neues Feld, roher HTML-Koerper), Sanitisierung erst beim Ausliefern (`mail/htmlSanitize.ts`, allowlist-basiert, kein `<script>`/`<style>`/Event-Handler), echte Wirkung fuer `blockRemoteImages` (Tracking-Pixel wird jetzt wirklich entfernt) und fuer die Klick-Zeit-Link-Pruefung (jeder Link wird auf `/link-check` umgeschrieben, jetzt im echten Klick-Fluss erreichbar). Als Nebeneffekt zwei weitere, unabhaengig dokumentierte Luecken geschlossen: `message_links` wird jetzt aus echten `<a href>`-Tags befuellt (Contract existierte seit Track 0, nie gebaut), `linkMismatchDetected`/`imageToTextRatio` koennen fuer empfangene HTML-Mail jetzt tatsaechlich ausloesen (vorher strukturell nie moeglich, da nur bodyText an die Klassifikation ging). Details siehe backend/README.md "HTML-Rendering des Mail-Bodies". Bewusst offen gelassen: Compose/Versand bleibt Klartext (war nicht Teil des Auftrags), `blockTrackingLinks` bleibt ohne eigene Wirkung (eigene Tracking-Redirect-Erkennung waere ein separater Schritt). Web-/iOS-UI fuer die Anzeige des HTML-Koerpers ist noch NICHT Teil dieses Schritts -- folgt als naechstes.
