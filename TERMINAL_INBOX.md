...
2. ~~**Wer macht externe Lookups?**~~ **Beantwortet (Web, 08.09., siehe SYNC.md):** einheitlich Track A als Nachbearbeitungsschritt nach `analyzeMail()`/`checkDraftForPhishing()`, `security-classification/` bleibt zustandslos, kein Contract-Bruch. Idealerweise als mockbare Adapter analog Track E. Wird jetzt in Track A umgesetzt.
3. ~~**`containsNewIban` — was heißt "neu"?**~~ **Beantwortet (Web, 08.09.):** "noch nie zuvor von diesem Absender an diesen User gesehen" — Abgleich in Track A gegen die IBAN-Historie in der eigenen DB.
