...
[2026-09-10] [offen] [Statusupdate + Rueckfrage] [echte Auth, naechster Schritt nach Persistenz] — Die 6-Punkte-Priority-Liste ("naechster Schritt", siehe WEB_INBOX.md) ist fertig (Commit `82aa28c` fuer den letzten Punkt, "Automatische Abmeldung bei Spam"). Massimo hat mich danach gebeten, mit dem naechsten, zuvor bereits abgestimmten Schritt weiterzumachen: echte Auth (Contract hat `security: bearerAuth` seit 08.09., Backend nutzte weiterhin nur `ensureDemoUser()`). Da es dazu noch keine detaillierte Spezifikation von euch gab, habe ich auf Massimos Wunsch ein eigenes, pragmatisches Design entworfen und umgesetzt statt vorher nachzufragen -- bitte gegenlesen, das hier ist bewusst als Ruecksprache gedacht, nicht als abgeschlossene Tatsache.

**Design (Details in `backend/README.md` "Auth"):**
- `sessions`-Tabelle (Opaque-Token, kein JWT), `middleware/auth.ts` (`requireAuth`) auf dem gesamten `/v1`-Router außer `routes/auth.ts`.
- `POST /accounts` (neuer Endpunkt, Contract-Luecke geschlossen -- `/auth/session` verwies seit Track 0 darauf, der Endpunkt selbst fehlte komplett): Login/Registrierung passiert implizit beim "Mail-Konto verbinden", find-or-create nach `emailAddress`, gibt einen Session-Token zurueck. `POST /auth/session` rotiert einen bestehenden Token.
- Autorisierung (nicht nur Authentifizierung): jede Route mit einer `:id` prueft zusaetzlich Besitz (`403`, wenn die Ressource einem anderen User gehoert) -- ging ueberraschend einfach, weil `folders`/`drafts`/`contracts` bereits eine `user_id`-Spalte hatten (nie durchgesetzt, da es bisher nur den einen Demo-User gab). `GET /messages`/`GET /contracts`/`GET /accounts` ohne ID filtern jetzt nach dem angemeldeten User statt ungefiltert alles zu liefern.
- Mit einem zweiten, echten User end-to-end verifiziert (Smoketest + manuell gegen echtes Postgres per curl): kann NICHT auf die Nachrichten/Ordner des ersten zugreifen.

**Bewusste Grenzen, die ich NICHT geloest habe (siehe backend/README.md fuer Details):**
1. Kein echter Gmail-OAuth-Code-Austausch/keine echte IMAP-Pruefung -- `POST /accounts` nimmt `provider`+`emailAddress`, ignoriert `oauthCode`/`imapPassword`.
2. Keine sichtbare Login-UI. Web meldet sich implizit mit einer festen Demo-Adresse an (`api.ts`, `ensureSessionToken()`). iOS unangetastet (MockAPIClient spricht ohnehin nie das Netzwerk an).
3. Klartext-Token, kein Rate-Limiting, kein vom Zugriffstoken getrennter Refresh-Token, kein Logout-Endpoint (nur Rotation, 30 Tage TTL).

**Zwei Funde nebenbei (beide dokumentiert, nur der erste behoben):**
- Mock-Server (`web/mock-server/server.mjs`) hatte `Access-Control-Allow-Headers: Content-Type` ohne `Authorization` -- der neue Header wurde vom Browser nach einer eigentlich erfolgreichen Preflight-Antwort trotzdem blockiert (sporadische, schwer greifbare Fehlschläge). Behoben.
- Beim Testen gegen eine echte, frische Postgres-Instanz faellt ein VORBESTEHENDER, unabhaengiger Bug auf: der Migrations-Smoketest-Block ("Ordner-Umbau-Migration") simuliert einen Alt-User mit `system_key='wichtig'` per direktem INSERT gegen das AKTUELLE Schema -- das verletzt die `CHECK`-Constraint, die 'wichtig' korrekt ablehnt (gegen `InMemoryStore` faellt das nicht auf, keine Constraints dort). Verifiziert, dass der Fehler bereits vor meinen Aenderungen existierte (gleicher Fehler auf dem vorherigen Commit). NICHT behoben (ausserhalb des Auth-Scopes) -- der Smoketest lief also bisher nie vollstaendig gegen eine echte Postgres-Instanz durch. Flagge das hier, falls das jemand vor mir uebersehen hat.

**Frage an euch:** passt der Ansatz "Login implizit ueber POST /accounts, keine sichtbare Login-UI vorerst" so, oder haettet ihr lieber einen expliziten `/login`-Endpunkt mit Passwort/Magic-Link gehabt? Aktuell ist "Auth" faktisch nur "stabile User-Identitaet + Session-Isolation", noch keine echte Zugriffskontrolle im Sinne von "wer darf sich ueberhaupt einloggen" (jede E-Mail-Adresse kann sich selbst anlegen). Falls das so nicht gewollt ist, bitte hier gegensteuern, bevor ich mehr darauf aufbaue.

**Tests:** Backend `npm run typecheck`/`npm test` gruen (neuer Auth-Testblock: 401-Faelle, Login-Idempotenz, Token-Rotation, Zwei-User-Isolation). Manuell gegen echtes Postgres per curl verifiziert (gleiche Faelle). Web `npm run build` gruen + End-to-End im Browser (impliziter Login beim Laden, keine sichtbare Aenderung fuer den User). iOS: kein Code-Change in diesem Schritt noetig (siehe oben) -- der zuvor bereits fertige, aber bis eben undokumentierte Schritt 5 (Abmeldung bei Spam) war schon in einem frueheren Durchlauf mit `xcodebuild` BUILD SUCCEEDED verifiziert, hier nur die README-Doku dazu nachgezogen.


[2026-09-10] [web] [Antwort auf beide offenen Punkte] — 

1) Login-Design: Timing-Ueberschneidung, war schon beantwortet (siehe SYNC.md "Verweis"-Eintrag von eben, dort verpasst weil zeitgleich geschrieben). Kurz nochmal direkt hier: JA, "implizit ueber Mail-Konto-Verbindung" ist der richtige Ansatz, kein separater /login mit Passwort/Magic-Link fuer v1 -- passt auch zum Onboarding-Leitprinzip (SYNC.md 10.09.: so wenig Schritte wie moeglich). Zur Zugriffskontroll-Luecke ("jede Adresse kann sich selbst anlegen"): ALLOWLIST fuer v1, kein freies Anlegen. Bitte serverseitige Allowlist-Pruefung ergaenzen (Tabelle/Config mit erlaubten E-Mail-Adressen, POST /accounts lehnt nicht-gelistete Adressen mit klarer Fehlermeldung ab). Echter Gmail-OAuth-Code-Austausch kommt noch -- Massimo richtet gerade das Google-Cloud-Projekt ein, Details/Redirect-URI/Scopes bitte klar kommunizieren sobald ihr soweit seid.

2) Vorbestehender Migrations-Smoketest-Bug (system_key='wichtig' verletzt CHECK-Constraint): bitte beheben, danke fuers Aufdecken statt Uebergehen. Der Test-Fixture muss auf die aktuellen System-Ordner (eingang/entwuerfe/gesendet/sonstiges/quarantaene/spam/papierkorb) aktualisiert werden -- vermutlich ein Rest von vor dem Ordner-Umbau, der nie nachgezogen wurde. Kein Contract-Aenderung, reiner Test-Fix.

Reihenfolge danach unveraendert: naechster funktionaler Punkt ist "Antworten ohne KI-Zwang" (Compose-Feld sofort nutzbar, KI-Entwurf nur optionaler Zusatz), siehe WEB_INBOX.md PRIORITAET-Eintrag 10.09. Optik-Punkte (Ordnername-Vorschlag, Header-Anzeige, Logo) bleiben danach.


[2026-09-21] [erledigt: Track A (efca792), Track F (24c08ca), Track C (a422e06)] [KORREKTUR - WEB_INBOX.md "ECHTE KI-ANBINDUNG" (Commit c3ec563) ist UEBERHOLT, bitte NICHT danach bauen] [contracts/ai-adapter-interface.ts + Track A/C/F] [Massimo hat direkt an Claude Code korrigiert, nicht ueber WEB_INBOX.md] — Falls hier parallel schon an der c3ec563-Version (driftmail-finanzierter Anthropic-Key server-seitig) gearbeitet wird: bitte stoppen, das ist durch die folgende Version ersetzt, nicht ergaenzt.

**Kernaenderung (Massimo woertlich, Anbieter-Reihenfolge war falsch gewichtet):** Geraete-eigene KI ist die PRIMAERE Quelle -- Apple Intelligence/Foundation Models auf iOS, browser-eigene On-Device-KI auf Web falls verfuegbar. KEIN driftmail-finanzierter Cloud-API-Key mehr (der in c3ec563 vorgeschlagene serverseitige `ANTHROPIC_API_KEY` auf driftmails Kosten entfaellt komplett). Cloud-KI (egal welcher Anbieter, nicht auf Anthropic festgelegt) nur, wenn der User selbst einen eigenen Zugang/API-Key in den Einstellungen eintraegt und das nutzen moechte -- dann auf seine eigenen Kosten, verschluesselt gespeichert (echtes AES-256-GCM, gleiches Muster wie `backend/src/auth/credentialsEncryption.ts` fuer OAuth-/IMAP-Zugangsdaten). Der Consent-Schritt vor jedem Cloud-Versand bleibt PFLICHT, unabhaengig davon wer zahlt (eigener Key aendert daran nichts).

Restlicher Umfang aus c3ec563 unveraendert gueltig: extractContract/summarize/draftReply sind die drei betroffenen Funktionen, draftReply bleibt bewusst nur Vorschlag (kein Auto-Send), kein Contract-Bruch (ai-adapter-interface.ts sieht das Shape schon vor).

Ich (Claude Code/Terminal) setze das jetzt ueber alle drei Tracks um (Backend: Provider-agnostisches Cloud-Adapter-Interface + verschluesselte Key-Speicherung + Settings-Endpunkt; iOS: Foundation-Models-Anbindung wo verfuegbar; Web: On-Device-Versuch falls Browser-API existiert, sonst ehrlich dokumentierter Verzicht) und dokumentiere wie gewohnt in SYNC.md. Meldet euch hier, falls daran schon parallel gearbeitet wurde, bevor doppelte/widerspruechliche Arbeit entsteht.


[2026-09-21] [beantwortet] [Re: "30 neue Diagnose-Probleme in 3 Dateien" -- iOS-Abschlussbericht Abwesenheitsassistent] — Kurze Einordnung, wie angefragt: das sind KEINE echten Compile-Fehler, sondern veraltete SourceKit-Index-Warnungen (Editor-Hintergrunddienst, nicht der echte Compiler) in `APIClient.swift`/`RemoteAPIClient.swift`/`MockAPIClient.swift` ("Cannot find type 'X' in scope" fuer laengst existierende Typen wie `MailAccount`/`Folder`/`Message"). Klassisches Symptom, wenn Xcodes SourceKit-Index nach dem manuellen Ergaenzen neuer Dateien in `project.pbxproj` kurz hinterherhinkt (dieses Projekt pflegt die `.pbxproj` von Hand, kein automatisches Xcode-Datei-Hinzufuegen). Selbst mit echtem `xcodebuild -project DriftmailApp.xcodeproj -scheme DriftmailApp build` gegengeprueft (nicht nur den Fork-Bericht geglaubt): **BUILD SUCCEEDED**, keine echten Fehler. Nichts zu tun, kein Blocker.

Ausserdem zur Klarstellung von eben (WEB_INBOX.md 21.09. "KLARSTELLUNG - Missverstaendnis aufgeklaert"): verstanden, Missverstaendnis war meins -- ich hatte "web, kuemmert sich um die seite bereits" so interpretiert, dass ihr direkt am Repo/an driftware.online baut. Ist jetzt klar: ihr schreibt nur Auftraege/Inhalte hierher, die Umsetzung (Seiten-Komponenten, Kachel-Link-Fix) macht immer Terminal. Nehme mir die beiden als-dringend markierten Punkte (Kachel-Link-Bug + Inhalts-Paket-Umsetzung) jetzt direkt vor.

---

[2026-09-25] [terminal] [Sammel-Info fuer Track F -- was heute auf iOS+Contract erledigt wurde, was fuer Web noch offen bleibt] — Ausfuehrlicher Sammel-Eintrag dazu in SYNC.md (heutiges Datum, "Status-Bericht — Sammel-Update"), hier nur die konkrete To-do-Liste fuer Track F, damit nichts uebersehen wird:

1. **Anbieter-Autoerkennung (WEB_INBOX.md 21.09.):** `contracts/mail-providers.json`/`api-spec.yaml` haben jetzt ein `domains`-Feld pro Provider (Commit `42e98ef`). `OnboardingScreen.tsx` matcht die eingegebene Adresse noch nicht dagegen -- iOS-Umsetzung (erst nur E-Mail-Feld, Domain-Match springt direkt ins passende Formular, unbekannte Domain faellt ohne Fehler auf generisches IMAP zurueck) als Referenz in `ios/README.md` 25.09.-Eintrag "Nachtrag: Anbieter automatisch aus E-Mail-Adresse erkennen".

2. **Design-Richtung "Outlook-inspiriert" (WEB_INBOX.md 24./25.09.):** Contract-Token (`design-tokens.json`) sind schon umgestellt (Commit `701f91b`) -- neuer Default-Akzent `#0078D4`/`outlook_blue`, neue `color.selected`/`color.light`-Werte, `radius.control/.card` auf 4px, `systemFolders.facetIconStyle` mit den vier Icon-Farbpaletten. Das ist nur die Farb-/Radius-Grundlage; das eigentliche Drei-Spalten-Layout (Ordner-Leiste/Liste/Lesebereich) ist der grosse, noch nicht begonnene Teil und liegt komplett bei euch.

3. **Vier Ordner-Icons (Eingang/Gesendet/Quarantaene/Papierkorb, WEB_INBOX.md 24./25.09.):** iOS hat eigene SVG-Nachbauten im 3D-Facetten-Stil gebaut (Commits `701f91b`, `e0e656e` fuer die Gesendet-Korrektur) -- Farbwerte + Motiv-Beschreibungen liegen als gemeinsame Referenz in `contracts/design-tokens.json` `systemFolders.facetIconStyle`, damit ihr nicht nochmal aus dem Chat-Verlauf rekonstruieren muesst. **Wichtige Lehre aus dem Gesendet-Icon:** unbedingt bei der TATSAECHLICHEN Ausgabegroesse pruefen (z.B. 24px), nicht nur in einer grossen Arbeitsvorschau -- ein erster Versuch sah bei 256px gut aus, wirkte bei echter Groesse aber wie ein duenner Strich.

4. **Gmail/googlemail.com (nur FYI, vermutlich fuer Web nicht relevant):** `gmail`-Provider-Eintrag hat jetzt zusaetzlich IMAP/SMTP-Presetwerte (Commit `47df48b`), weil Gmail-OAuth auf iOS nicht funktioniert und der Fallback sonst im leeren generischen Formular landete. `authType` bleibt `"oauth"` -- Web nutzt weiterhin den echten, funktionierenden Google-Login und braucht diesen IMAP-Fallback vermutlich nicht, hier nur der Vollstaendigkeit halber erwaehnt, falls doch relevant.
