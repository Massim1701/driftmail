...
1. **Kein Auth-/User-Konzept in api-spec.yaml** (Track A) — kein `userId` in Pfaden/Query/Body. Backend läuft aktuell mit festem Demo-User. Soll Auth noch in v1 rein oder bewusst später?
2. **Wer macht externe Lookups?** (Track A+B, mehrfach dieselbe Kategorie) — `senderDomainAgeDays`/`domainReputationScore` (WHOIS), `ipReputationFlag` (Botnetz-Blocklist), `recipientReputation` (fraud_alerts/DB). `security-classification/` ist bewusst zustandslos und kann das nicht selbst. Vermutlich Track A nach dem jeweiligen Aufruf, aber nicht selbst entscheidbar (plattformübergreifend) — braucht das Interface dafür einen optionalen Lookup-Schritt/Adapter?
3. **`containsNewIban` — was heißt "neu"?** (Track B) Impliziert Abgleich gegen IBAN-Historie des Absenders, den ein zustandsloses Modul nicht machen kann. Wo passiert der Abgleich?
4. **DB-Tabellen ohne API-Entsprechung** (Track A) — `unsubscribe_actions`, `message_links`, `reminders`, `signatures`, `security_audit_log`, `ai_provider_config` haben keine Pfade in api-spec.yaml. Bewusst für später, oder fehlen Endpunkte (betrifft v.a. Track D/E)?
