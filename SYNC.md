...
- [2026-09-08] [terminal] [A] Beim Gesamt-Check gegen `api-spec.yaml` gefunden: `send_abuse_flags` (Bot/Human-Missbrauchserkennung beim Versand, WEB_INBOX.md 08.09. "Ausgehender Phishing-Check im Composer") existiert bisher **weder** als `CREATE TABLE` in `contracts/db-schema.sql` noch als Endpoint in `contracts/api-spec.yaml` — nur als SQL-`ALTER`-Vorschlag (für eine noch gar nicht existierende Tabelle) in `WEB_INBOX.md`. Kein Blocker für Track A (kein eigener Versand-Pfad in diesem Durchstich), aber Track B/Contract müssen das erst als vollständigen Contract nachziehen (`CREATE TABLE send_abuse_flags` + ggf. Endpunkt), bevor Track A das Auto-Delete-artige Verhalten (`action_taken = 'send_blocked'` bei `flag_reason = 'phishing_content'`) real implementieren kann. `containsSensitiveData`/`recipientReputation`/`riskyLinks` in `POST /messages/draft/phishing-check` sind davon unberührt und bereits umgesetzt (Mock, siehe Änderungsprotokoll unten).
- [2026-09-08] [terminal] [A] `recipientReputation` in `POST /messages/draft/phishing-check` ist sowohl bei Track B (`security-classification/`) als auch jetzt im Backend-Mock immer `"unknown"`. Grund im Backend: `fraud_alerts` (`db-schema.sql`) ist noch nicht in `src/types.ts`/`src/db/store.ts` modelliert. Wer den echten Lookup baut (Track A gegen `fraud_alerts` nach dem `checkDraftForPhishing`-Aufruf, wie in Track B's README vermutet) und wann, ist noch offen — Teil des noch nicht gestarteten Integrations-Schritts.
